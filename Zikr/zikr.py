...
Requirements:
    ...
    - (Optional) Termux:API app for vibration on Android/Termux
...
# ---------------- Termux vibration helpers ----------------

def is_termux() -> bool:
    """Check if running inside Termux on Android."""
    return os.path.isdir(os.path.expanduser("~/.termux")) or "TERMUX_VERSION" in os.environ

def has_termux_api() -> bool:
    """Check if termux-api package (termux-vibrate etc.) is installed."""
    return shutil.which("termux-vibrate") is not None

def vibrate(duration: int = 100) -> None:
    """Vibrate device if inside Termux and termux-api is installed."""
    if is_termux() and has_termux_api():
        try:
            subprocess.run(["termux-vibrate", "-f", "-d", str(duration)], check=False)
        except Exception:
            pass  # Ignore vibration errors silently
